...
export class YoutubeAuthorizedApi {
  ...
  private async createStream(
    ...
    const res = await google
      .youtube({ version: "v3", auth: this.oAuth2Client })
      .liveStreams.insert({
  ...
  async createBroadcast(
    ...
    if (res.statusText !== "OK" || !translationId) {
      console.log(
        "[YOUTUBE API][createBroadcast] response",
  ...
  async bindBroadcastToStream(broadcastId: string, streamInfo: StreamInfo) {
    ...
    if (res.statusText !== "OK") {
      console.log(
        "[YOUTUBE API][bindBroadcastToStream] response",
        JSON.stringify(res, null, 2),
      );
  ...
  async stopBroadcast(broadcastId: string) {
    ...
    if (res.statusText !== "OK") {
      console.log(
        "[YOUTUBE API][stopBroadcast] error",
        JSON.stringify(res, null, 2),
      );
    }
  ...
  private async deleteBroadcast(broadcastId: string) {
    ...
    if (res.statusText !== "OK") {
      console.log(
        "[YOUTUBE API][deleteBroadcast] error",
        JSON.stringify(res, null, 2),
      );
    }
